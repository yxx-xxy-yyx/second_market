      ...
      <el-card class="review-card" v-loading="loading">
        <div class="order-info" v-if="orderInfo">
          ...
          <div class="product-details">
            ...
            <div class="product-price">¥{{ orderInfo.amount }}</div>
          </div>
        </div>
...
const getProductImage = (images) => {
  if (!images) return 'https://via.placeholder.com/100?text=No+Image'
  try {
    const imageArray = typeof images === 'string' ? JSON.parse(images) : images
    return imageArray[0] || 'https://via.placeholder.com/100?text=No+Image'
  } catch {
