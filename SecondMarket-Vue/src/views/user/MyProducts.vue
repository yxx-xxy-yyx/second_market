...
.equal-height-row {
  display: flex;
  align-items: stretch; /* 核心：让左右 Col 高度一致 */
  flex-wrap: wrap; /* 保证移动端能正常换行 */
}
...
.table-card.fixed-height-card :deep(.el-card__body) {
  ...
  flex-direction: column;
  height: 100%;
  min-height: 0; /* 允许内部收缩，防止切换语言时长文字撑破框 */
  padding: 20px;
}
...
.action-button-grid .el-button {
  ...
  margin: 0 !important; /* 清除Element Plus默认的margin */
...
@media (max-width: 768px) {
  ...
  .filter-left .el-input, .filter-right .el-select {
    width: 100% !important;
  }
}
</style>
