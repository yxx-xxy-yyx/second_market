          ...
          <div class="step-content">
            ...
            <div v-show="currentStep === 1" class="step-card">
              <div v-if="analyzing" class="analyzing-container">
                <el-icon class="analyzing-icon" :size="80"><Loading /></el-icon>
              ...
              <div v-else-if="analysisResult" class="analysis-result">
                <div class="result-header">
                  <el-icon class="result-icon" color="#67c23a"><CircleCheck /></el-icon>
                ...
                <el-row :gutter="15" class="result-cards-grid">
                  ...
                  <el-col :xs="24" :sm="24" :md="10" :lg="10">
                    ...
                    <el-card class="result-mini-card condition-card" shadow="hover">
                      <div class="mini-card-icon" style="background: linear-gradient(135deg, #67c23a 0%, #85ce61 100%);">
                        <el-icon :size="24"><Stamp /></el-icon>
                      </div>
                      <div class="mini-card-content">
                        <div class="mini-card-label">{{ $t('publish.conditionScore') }}</div>
                        <div class="mini-card-value">{{ analysisResult.conditionScore }}<span class="value-unit">{{ $t('publish.scoreUnit') }}</span></div>
                        <el-progress
                          :percentage="analysisResult.conditionScore * 10"
                          :stroke-width="6"
                          :show-text="false"
                          :color="getConditionColor(analysisResult.conditionScore)"
                        />
                        <div class="mini-card-desc">{{ analysisResult.conditionDescription || analysisResult.conditionExplanation }}</div>
                      </div>
                    </el-card>

                    <!-- 价格建议卡片 -->
                    <el-card class="result-mini-card price-card" shadow="hover">
                      <div class="mini-card-icon" style="background: linear-gradient(135deg, #ff9800 0%, #ffc107 100%);">
                        <el-icon :size="24"><Money /></el-icon>
                      </div>
                      <div class="mini-card-content">
                        <div class="mini-card-label">{{ $t('publish.priceSuggest') }}</div>
                        <div class="mini-card-value price-value">¥{{ analysisResult.minPrice }} - ¥{{ analysisResult.maxPrice }}</div>
                    ...
                    <el-card class="result-mini-card suggest-card" shadow="hover">
                      <div class="mini-card-icon" style="background: linear-gradient(135deg, #409eff 0%, #66b1ff 100%);">
                  ...
                  >
                    ...
                    <el-form-item :label="$t('publish.form.titleKo')" prop="titleKo">
                      <el-input
                        ...
                        :placeholder="$t('publish.form.titleKoPlaceholder')"
                        maxlength="100"
                        show-word-limit
                      />
                    </el-form-item>
                    ...
                    <el-form-item :label="$t('publish.form.category')" prop="category">
                      <el-select v-model="form.category" :placeholder="$t('publish.form.categoryPlaceholder')" style="width: 100%">
                        <el-option
                          v-for="cat in categories"
                          :key="cat.id"
                          :label="locale === 'zh' ? cat.name : $t('categories.' + cat.id)"
                          :value="cat.name"
                        />
                      </el-select>
                    </el-form-item>
                    ...
                    <el-form-item :label="$t('publish.form.school')" prop="schoolId">
                      <el-select v-model="form.schoolId" :placeholder="$t('publish.form.schoolPlaceholder')" style="width: 100%">
                        <el-option
                          v-for="s in schoolStore.schoolList"
                          :key="s.value"
                          :label="s.label"
                          :value="s.value"
                        />
                      ...
                      <el-col :span="12">
                        <el-form-item :label="$t('publish.form.price')" prop="price">
                          <el-input
                            ...
                            :placeholder="$t('publish.form.pricePlaceholder')"
                            type="number"
                            step="0.01"
                          >
                      ...
                      <el-col :span="12">
                        <el-form-item :label="$t('publish.form.originalPrice')" prop="originalPrice">
                          <el-input
                            ...
                            :placeholder="$t('publish.form.originalPricePlaceholder')"
                            type="number"
                            step="0.01"
                          >
                            <template #prepend>¥</template>
                    ...
                    <el-form-item :label="$t('publish.form.desc')" prop="description">
                      <el-input
                        ...
                        type="textarea"
                        :rows="4"
                        :placeholder="$t('publish.form.descPlaceholder')"
                        maxlength="500"
                        show-word-limit
                      />
                    </el-form-item>
                    ...
                    <el-form-item :label="$t('publish.form.descKo')" prop="descriptionKo">
                      <el-input
                        ...
                        type="textarea"
                        :rows="4"
                        :placeholder="$t('publish.form.descKoPlaceholder')"
                        maxlength="500"
                        show-word-limit
                      />
                    </el-form-item>
                    ...
                    <el-form-item :label="$t('publish.form.conditionDesc')" prop="conditionDesc">
                      <el-input
                        ...
                        type="textarea"
                        :rows="3"
                        :placeholder="$t('publish.form.conditionDescPlaceholder')"
                        maxlength="200"
                        show-word-limit
                      />
                    </el-form-item>
                    ...
                    <el-card class="preview-card" shadow="hover">
                      ...
                      <div class="preview-content">
                        ...
                        <div class="preview-price-row">
                          ...
                          <span class="preview-original-price" v-if="form.originalPrice">¥{{ form.originalPrice }}</span>
                        </div>
      ...
      <el-col :xs="24" :sm="24" :md="7" :lg="7">
        <div class="sidebar-area">
          ...
          <el-card class="guide-card" shadow="hover">
            ...
            <el-timeline>
              <el-timeline-item :timestamp="$t('publish.step1Text')" :hollow="currentStep !== 0" :type="currentStep > 0 ? 'success' : 'primary'">
                {{ $t('publish.guideStep1Title') }}
                <div class="timeline-desc">{{ $t('publish.guideStep1Desc') }}</div>
              </el-timeline-item>
              <el-timeline-item :timestamp="$t('publish.step2Text')" :hollow="currentStep !== 1" :type="currentStep > 1 ? 'success' : 'primary'">
                {{ $t('publish.guideStep2Title') }}
                <div class="timeline-desc">{{ $t('publish.guideStep2Desc') }}</div>
              </el-timeline-item>
              <el-timeline-item :timestamp="$t('publish.step3Text')" :hollow="currentStep !== 2" :type="currentStep > 2 ? 'success' : 'primary'">
...
@media (max-width: 768px) {
  ...
  .publish-steps { padding: 0 20px; }
  :deep(.el-upload--picture-card),:deep(.el-upload-list__item) { width: 120px; height: 120px; }
  .button-group { flex-direction: column; }
  .button-group .el-button { width: 100%; }
  .sidebar-area { position: relative; top: 0; margin-top: 20px; }
}

.sidebar-area { position: sticky; top: 20px; }

.guide-card,.stats-card,.ai-result-card,.tips-card {
  border: 1px solid #ebeef5;
  transition: all 0.3s ease;
}

.guide-card:hover,.stats-card:hover,.ai-result-card:hover,.tips-card:hover {
  box-shadow: 0 4px 12px rgba(64, 158, 255, 0.12);
  border-color: #c6e2ff;
  transform: translateY(-2px);
}
...
.ai-result-item { margin: 10px 0; }
.result-label { font-size: 12px; color: #909399; margin-bottom: 6px; font-weight: 500; }
.result-value { font-size: 14px; color: #303133; font-weight: 500; line-height: 1.6; }
.result-value.price { color: #f56c6c; font-size: 18px; font-weight: 600; }
