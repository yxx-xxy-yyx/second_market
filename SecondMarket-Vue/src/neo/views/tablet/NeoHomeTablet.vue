<template>
  <div class="space-y-5">
    <div class="rounded-2xl overflow-hidden border border-gray-100 bg-white shadow-sm">
      <div class="px-5 py-6 bg-gradient-to-br from-[rgba(var(--primary-rgb),0.96)] to-[rgba(var(--secondary-rgb),0.92)] text-white">
        <div class="text-[20px] font-semibold tracking-wide">{{ $t('neo.home.heroTitle') }}</div>
          ...
          <el-skeleton :loading="loading" animated>
            <template #template>
              <div class="grid grid-cols-3 gap-3">
                <div v-for="n in 3" :key="n" class="h-[160px] rounded-2xl bg-gray-100" />
              </div>
            </template>
            <template #default>
              <div class="grid grid-cols-3 gap-3">
                <button
                  v-for="p in aiRecommended.slice(0, 6)"
                  :key="p.id"
                  class="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden text-left hover:shadow-md transition"
                  @click="goDetail(p.id)"
                >
                  <img :src="getProductImage(p.images)" class="w-full h-[104px] object-cover bg-gray-50" @error="handleImgError" />
                  <div class="p-3">
                    <div class="text-[13px] font-semibold text-gray-900 line-clamp-2 min-h-[34px]">
                      {{ p.title }}
                    </div>
                    <div class="mt-2 text-[14px] font-bold text-[color:var(--primary-color)]">
                      {{ currency }}{{ p.price }}
                    </div>
                  </div>
                </button>
