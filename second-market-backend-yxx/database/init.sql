...
CREATE TABLE IF NOT EXISTS `ai_feedback` (
    ...
    `feature` VARCHAR(50) NOT NULL COMMENT '功能模块: analysis/authenticate/trend/search/campus/trust/dispute',
    `target_id` BIGINT DEFAULT NULL COMMENT '目标记录ID',
    `rating` TINYINT DEFAULT 0 COMMENT '用户评分 1-5',
    `helpful` TINYINT DEFAULT 1 COMMENT '是否有用 0/1',
    `feedback_text` VARCHAR(500) DEFAULT NULL COMMENT '反馈文本',
    `ai_response` TEXT DEFAULT NULL COMMENT 'AI 输出快照',
...
CREATE TABLE IF NOT EXISTS `ai_market_trend_stats` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    ...
    `avg_price` DECIMAL(12,2) DEFAULT NULL COMMENT '均价',
    `min_price` DECIMAL(12,2) DEFAULT NULL COMMENT '最低价',
    `max_price` DECIMAL(12,2) DEFAULT NULL COMMENT '最高价',
    `sold_count` INT DEFAULT 0 COMMENT '成交数量',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_cat_school_date` (`category`, `school_id`, `stat_date`),
    KEY `idx_category` (`category`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = 'AI行情统计表';
...
CREATE TABLE IF NOT EXISTS `ai_suggestion` (
    ...
    `score` DECIMAL(3,2) DEFAULT NULL COMMENT '分数',
    `source` VARCHAR(50) DEFAULT NULL COMMENT '来源: llm/rule/statistics',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    KEY `idx_product_id` (`product_id`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = 'AI建议表';

CREATE TABLE IF NOT EXISTS `ai_search_log` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `user_id` BIGINT DEFAULT NULL COMMENT '用户ID',
    `query` VARCHAR(255) DEFAULT NULL COMMENT '搜索关键词',
    `category` VARCHAR(100) DEFAULT NULL COMMENT '搜索分类',
    `result_count` INT DEFAULT 0 COMMENT '结果数量',
    `click_count` INT DEFAULT 0 COMMENT '点击数量',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    KEY `idx_user_id` (`user_id`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = 'AI搜索日志表';

-- ===== product 表补齐可选索引 =====

ALTER TABLE `product` ADD INDEX IF NOT EXISTS `idx_product_category_school` (`category`, `school_id`);
ALTER TABLE `product` ADD INDEX IF NOT EXISTS `idx_product_status_school` (`status`, `school_id`);
ALTER TABLE `product` ADD INDEX IF NOT EXISTS `idx_product_user_id` (`user_id`);
