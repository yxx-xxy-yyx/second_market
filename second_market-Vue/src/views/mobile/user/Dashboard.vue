  ...
  <div class="user-dashboard">
    <div class="dashboard-container">
      ...
      <div class="notice-bar-wrapper">
        ...
        <el-carousel height="44px" direction="vertical" :autoplay="true" :interval="3000" indicator-position="none"
          arrow="never" class="notice-carousel">
      ...
      <div class="dashboard-tab-panel-wrapper px-3 mb-10">
        <div class="dashboard-tabs flex items-center gap-2 overflow-x-auto mb-4">
          <button v-for="tab in dashboardTabs" :key="tab.key" @click="activeDashboardTab = tab.key"
            :class="['dashboard-tab-btn flex-1 min-w-[80px] rounded-2xl px-3 py-2 text-sm font-medium transition', activeDashboardTab === tab.key ? 'bg-cyan-600 text-white shadow-md' : 'bg-gray-100 text-gray-700']">
            {{ tab.label }}
          </button>
        </div>

        <div class="dashboard-tab-content space-y-4">
          <div v-if="activeDashboardTab === 'ai'">
            <div class="ai-recommend-bar" @click="toggleAiAssistant">
              <div class="ai-bar-content">
                <el-icon class="ai-bar-icon">
                  <MagicStick />
                </el-icon>
          ...
          <div v-if="activeDashboardTab === 'hot'">
            <div class="hot-products-section bg-white rounded-2xl p-4 shadow-sm" v-loading="loading">
              <div class="section-header">
                <div class="header-left">
                  <el-icon class="title-icon" color="#ff6b6b" :size="20">
                    <TrendCharts />
                  </el-icon>
                  <h2 class="section-title">{{ $t('dashboard.hotProducts') }}</h2>
                </div>
                <el-link type="primary" @click="handleViewMore('hot')" class="view-more-link">
                  {{ $t('dashboard.viewMore') }} <el-icon size="12">
                    <ArrowRight />
                  </el-icon>
                </el-link>
              </div>
              <div class="horizontal-scroll-container">
                <div class="horizontal-scroll-content">
                  <div v-for="product in hotProducts.slice(0, 10)" :key="product.id" class="horizontal-product-card"
                    ...
                    <div class="horizontal-product-info">
                      <div class="horizontal-product-title">{{ locale === 'ko' && product.titleKo ? product.titleKo :
                        product.title }}</div>
          ...
          <div v-if="activeDashboardTab === 'recommend'">
            <div class="recommend-section bg-white rounded-2xl p-4 shadow-sm" v-loading="loading">
              <div class="section-header">
                <div class="header-left">
                  <el-icon class="title-icon" color="#409eff" :size="20">
                    <Goods />
                  </el-icon>
              ...
              <el-row :gutter="8">
                <el-col v-for="product in recommendProducts.slice(0, 12)" :key="product.id" :xs="12" :sm="12">
                  <div class="product-card-compact" @click="handleProductClick(product.id)">
                    ...
                    <div class="product-info-compact">
                      <div class="product-title-compact">{{ locale === 'ko' && product.titleKo ? product.titleKo :
                        product.title }}</div>
          ...
          <div v-if="activeDashboardTab === 'trade'">
            <div class="trade-dynamic-section bg-white rounded-2xl p-4 shadow-sm">
              <div class="section-header">
                <div class="header-left">
                  <el-icon class="title-icon" color="#67c23a" :size="18">
                    <Clock />
                  </el-icon>
    ...
    <!-- 公告详情抽屉 - APP风格 -->
    <el-drawer v-model="showNoticeDrawer" :title="$t('dashboard.notice')" direction="rtl" size="85%" :withHeader="true"
      class="app-drawer">
      <div class="notice-drawer-content">
        <div v-for="(notice, index) in allNotices" :key="index" class="notice-drawer-item"
    ...
    <!-- 公告详情对话框 - APP风格 -->
    <el-dialog v-model="showNoticeDetail" :title="selectedNotice?.title" width="90%" class="app-dialog"
      :border-radius="16">
...
const activeDashboardTab = ref('ai')
const dashboardTabs = ref([
  { key: 'ai', label: t('dashboard.aiTitle') },
  { key: 'hot', label: t('dashboard.hotProducts') },
  { key: 'recommend', label: t('dashboard.recommend') },
  { key: 'trade', label: t('dashboard.tradeDynamic') }
])
