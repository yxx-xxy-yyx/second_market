<template>
  <div class="px-3 py-4 space-y-4">
    <div class="flex items-center gap-3 mb-1">
      <el-icon :size="22" class="text-gray-700 cursor-pointer active:scale-90 transition-transform" @click="router.back()"><ArrowLeft /></el-icon>
      <div class="text-base font-semibold text-gray-900">{{ $t('publish.title') }}</div>
    </div>
    <div class="rounded-2xl bg-white border border-gray-100 shadow-sm p-4">
      ...
      <div class="text-xs text-gray-500 mt-1">{{ $t('publish.uploadTip') }}</div>
    </div>
    ...
    <div v-if="analysisResult" class="rounded-2xl bg-white border border-gray-100 shadow-sm p-4">
      <div class="flex items-center justify-between">
        <div class="text-sm font-semibold text-gray-900">{{ $t('publish.aiResult') }}</div>
        <button class="text-sm sm-text-primary" @click="handleAutoFill">{{ $t('publish.autoFill') }}</button>
      </div>
      ...
      <div class="mt-3 text-xs text-gray-600 leading-relaxed line-clamp-3">
        {{ analysisResult.description || '' }}
      </div>
    </div>

    <div class="rounded-2xl bg-white border border-gray-100 shadow-sm p-4">
      <div class="text-sm font-semibold text-gray-900 mb-3">{{ $t('publish.step3') }}</div>
      ...
      <el-form ref="formRef" :model="form" :rules="formRules" label-position="top">
        ...
        <el-form-item :label="$t('publish.conditionScore')" prop="conditionScore">
