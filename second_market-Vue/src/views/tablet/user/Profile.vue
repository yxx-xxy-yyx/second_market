<template>
  <div class="px-4 py-4 space-y-5">
    <div class="rounded-2xl overflow-hidden border border-gray-100 shadow-sm bg-white">
      <div class="px-5 py-6 bg-gradient-to-br text-white" :style="{ background: 'linear-gradient(135deg, rgba(6,182,212,1) 0%, rgba(139,92,246,0.92) 100%)' }">
        <div class="flex items-center gap-5">
          <button class="relative" @click="showAvatarDialog = true">
            <el-avatar :size="92" :src="avatarUrl" class="ring-4 ring-white/40">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="mt-1 text-sm text-white/85 truncate">
              UID: {{ displayUid }}
            </div>
            <div class="mt-3 grid grid-cols-3 gap-3">
              ...
              <div class="rounded-2xl bg-white/20 px-3 py-3 text-center">
                <div class="text-xl font-bold">{{ statistics.productCount || 0 }}</div>
                <div class="text-xs text-white/85 mt-1">{{ $t('nav.myProducts') }}</div>
              </div>
              <div class="rounded-2xl bg-white/20 px-3 py-3 text-center">
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-4 py-3 text-sm font-semibold text-gray-900 bg-gray-50">{{ $t('common.userCenter') }}</div>
        <button class="w-full px-4 py-4 flex items-center justify-between active:bg-gray-50 transition" @click="router.push('/user/my-products')">
          <div class="flex items-center gap-3">
            <div class="h-10 w-10 rounded-2xl sm-bg-primary-soft sm-text-primary flex items-center justify-center">
              <el-icon :size="20"><Box /></el-icon>
            </div>
            <div class="text-sm font-semibold text-gray-900">{{ $t('nav.myProducts') }}</div>
          </div>
      ...
      <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-4 py-3 text-sm font-semibold text-gray-900 bg-gray-50">{{ $t('common.more') }}</div>
    ...
    <el-button type="danger" class="w-full" @click="handleLogout">{{ $t('common.logout') }}</el-button>
  </div>

  <el-dialog v-model="showAvatarDialog" :title="$t('profile.changeAvatar')" width="620px" :close-on-click-modal="false">
    <div class="py-4 flex justify-center">
      <AvatarUpload :model-value="avatarUrl" :size="200" @success="handleAvatarSuccess" />
    </div>
  </el-dialog>
