  ...
  <div class="notice-page">
    <div class="page-container">
      ...
      <div class="page-header">
        <div class="relative flex justify-center items-center mb-1 w-full">

          <el-icon @click.stop="router.back()" :size="28"
            class="absolute left-0 cursor-pointer text-gray-600 md:hidden z-50 active:text-blue-500 p-1">
            <ArrowLeft />
          </el-icon>
        ...
        <p class="page-desc text-center">{{ locale === 'ko' ? '플랫폼 소식과 중요 안내를 확인하세요' : '及时了解平台动态和重要通知' }}</p>
      </div>

      <div class="notice-list" ref="noticeListRef" v-loading="loading">
        <div v-for="(notice, index) in noticeList" :key="notice.id" class="notice-item" @click="toggleNotice(notice)">
            ...
            <div class="notice-left">
              ...
              <div class="notice-info">
                <div class="notice-title-row">
                  ...
                  <el-tag v-if="index < 3" type="danger" size="small" class="new-tag">{{ locale === 'ko' ? '최신' : '最新'
                  }}</el-tag>
                </div>
          ...
          <transition name="expand">
            <div v-if="notice.expanded" class="notice-content">
              ...
              <div class="content-body"
                v-html="locale === 'ko' && notice.contentKo ? notice.contentKo : notice.content"></div>
            </div>
          </transition>
        </div>
        ...
        <div v-if="isMobileOrTablet && !loading && !noMoreData" class="load-more-tip">加载更多...</div>
        <div v-if="isMobileOrTablet && noMoreData" class="no-more-tip">没有更多数据了</div>
      </div>

      <div class="pagination" v-if="!isMobileOrTablet && noticeList.length > 0">
        <el-pagination v-model:current-page="pagination.page" v-model:page-size="pagination.size"
          :page-sizes="[5, 10, 20]" :total="pagination.total" layout="total, sizes, prev, pager, next, jumper"
...
import { ElMessage, ElIcon, ElTag, ElEmpty, ElPagination } from 'element-plus'
import { useI18n } from 'vue-i18n'
import { useRouter } from 'vue-router'
import { ArrowDown, Bell, Flag, Tools } from '@element-plus/icons-vue'
...
const getTypeIcon = (type) => {
  const iconMap = { system: Bell, activity: Flag, maintenance: Tools }
...
const getTypeTagType = (type) => {
  const typeMap = { system: 'primary', activity: 'success', maintenance: 'warning' }
...
const handleScroll = () => {
  if (!isMobileOrTablet.value || loading.value || noMoreData.value) return
  const el = noticeListRef.value
...
const debounce = (fn, delay = 100) => {
  return (...args) => {
    ...
    scrollTimer = setTimeout(() => fn(...args), delay)
  }
}
...
onMounted(() => {
  ...
  noticeListRef.value?.addEventListener('scroll', debounce(handleScroll))
})

onUnmounted(() => {
  window.removeEventListener('resize', checkDevice)
  noticeListRef.value?.removeEventListener('scroll', debounce(handleScroll))
})
</script>
